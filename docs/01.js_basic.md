...
- 2024년 06월: ECMAScript 2024

---

## 자바스크립트로 할 수 있는 일
- 참고: https://ko.javascript.info/intro#ref-1315
...
### HTML5
- 데이터관리(Web Storage), 실시간 통신(Web Socket), 위치추적(Geolocation API), 멀티스레드(Web Workers), 디바이스 제어(카메라, 마이크, 진동 등)
...
```javascript
명령문1;                     alert("Hello World!");
명령문2                      alert("Hello ")
명령문3;                     alert("World!!");
명령문4; 명령문5;             alert("Hello "); alert("World!!!");
```
...
### 식별자 명명 규칙
- 변수명, 함수명, 클래스명 등의 이름을 지을 때 사용하는 규칙
- 영문자, 숫자, `_`, `$`
- 첫 글자는 영문자, `_`, `$`만 가능
- 유니코드 문자 사용 가능
...
- 타입이 다르면 false를 반환하고 타입이 같을 경우는 다음 규칙을 따름
  ...
  - 모두 null이거나 모두 undefined일 경우 true, null과 undefined는 false
    ...
    ```
  - 같은 객체를 참조할 경우 true, 같은 프로퍼티를 가진 객체라도 다른 객체를 참조할 경우 false
    ```javascript
    [10, 20] === [10, 20] // false
    ```
...
### 동등 연산자
- 동등 연산자(`==`, equal)
  - 두 데이터의 타입이 다르면 적절히 형변환한 후 일치비교(`===`)
  - 타입이 달라도 값이 같으면 true
...
- 두 값의 타입이 다를 경우
  ...
  - true는 1로 false는 0으로 변환한 후 비교
    ...
    ```
  - 객체와 숫자(또는 문자열) 비교시 객체의 valueOf() 먼저 찾고 없으면 toString() 메서드를 이용 기본값으로 변환한 후 비교. 단, Date는 toString()으로 바로 변환한다.
    ```javascript
    1 == new Number(1) -> 1 === 1 -> true
    ```
...
- switch에 지정한 비교값과 매칭되는(`===`) case 구문 수행
  - 일치하는 case가 없을 경우 default 구문 실행(선택사항)
  - break 구문을 만날 때까지 또는 마지막 case, default 구문까지 실행
...
- 속성의 값으로 모든 데이터 타입 지정 가능
  - 값으로 함수가 지정된 속성을 메서드(method)라 함
- 여러개의 속성을 포함할 수 있음
...
- 객체의 속성과 기능에 접근할 때는 dot 표기법( . )을 사용하거나 대괄호 표기법( ['속성명'] )을 사용
